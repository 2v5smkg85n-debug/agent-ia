...
def get_recommandations():
    ...
    # Strategies a eviter: bloque AGRESSIVEMENT les strategies perdantes
    # Seuils stricts: des 3 trades, WR < 55% OU PnL < -3EUR = bloque
    # Seul pattern_reversal (65% WR, +20EUR) devrait survivre
    for strat, stats in learning.get("stats_strategies", {}).items():
        _n = stats.get("n", 0)
        _wr = stats.get("win_rate", 0)
        _pnl = stats.get("pnl_total", 0)
        if _n >= 3 and (_wr < 55 or _pnl < -3):
